...
{
    [AddComponentMenu("UI/UI Scroll Rect", 37)]
    [SelectionBase]
    [ExecuteInEditMode]
    [DisallowMultipleComponent]
    [RequireComponent(typeof(RectTransform))]
    public class UIScrollRectFaster : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        public UIScrollRectFaster Counterpart;
        public bool isLeftCounterpart;

        public TMP_InputField YFactorInput;
        public TextMeshProUGUI AllowUpdateTMP;
        public TextMeshProUGUI VelocityTMP;
        public TextMeshProUGUI DraggingTMP;
        ...
        {
            to = isLeftCounterpart ? 0 : -(m_Content.sizeDelta.x / 2);
        }
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            ...
            float positionX = m_ContentStartPosition.x + pointerDelta.x;

            // Offset to get content into place in the view.
            float offset = positionX - m_Content.anchoredPosition.x;
            positionX += offset;
            if (m_MovementType == MovementType.Elastic)
            {
                if (offset != 0)
                    positionX -= RubberDelta(offset, m_ViewBounds.size.x);
            }

            positionX = positionX > 0 ? 0 : positionX < -(m_Content.sizeDelta.x / 2) ? -(m_Content.sizeDelta.x / 2) : positionX;

            SetContentAnchoredPosition(new Vector2(positionX, 0));
        }
        ...
        private void Update()
        {
            if (AllowUpdateTMP != null)
                AllowUpdateTMP.text = $"AllowUpdate: {allowUpdate}";
            if (VelocityTMP != null)
                VelocityTMP.text = $"Velocity: {m_Velocity.x}";
            if (DraggingTMP != null)
                DraggingTMP.text = $"Dragging: {m_Dragging}";
        ...
        public void NavigateBackFromDMs()
        {
            Vector2 position = m_Content.anchoredPosition;
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (!m_Dragging && Math.Round(position.x) != 0 && Math.Round(position.x) != Math.Round(-(m_Content.sizeDelta.x / 2)))
            {
                for (int axis = 0; axis < 1; axis++)
                {
                    if (allowUpdate)
                    {
                        if ((isLeftCounterpart && -m_Velocity.x > VelocityThreshold) || (!isLeftCounterpart && m_Velocity.x > VelocityThreshold))
                        {
                            to = isLeftCounterpart ? -(m_Content.sizeDelta.x / 2) : 0;
                        }
                        else
                        {
                            to = m_ContentBounds.center.x < 0 ? -(m_Content.sizeDelta.x / 2) : 0;
                        }
                    ...
                    if (m_MovementType == MovementType.Elastic && (position.x != 0 || position.x != -(m_Content.sizeDelta.x / 2)))
                    {
                        float speed = m_Velocity[axis];
                        position[axis] = Mathf.SmoothDamp(m_Content.anchoredPosition[axis], to, ref speed, m_Elasticity, Mathf.Infinity, deltaTime);
            ...
            if (!allowUpdate && (Math.Round(position.x) == 0 || Math.Round(position.x) == Math.Round(-(m_Content.sizeDelta.x / 2))))
            {
                allowUpdate = true;

                if ((isLeftCounterpart && to != 0) || (!isLeftCounterpart && to == 0))
                {
                    enabled = false;
        ...
        private float CalculateOffset(float delta)
        {
            ...
            Vector2 max = m_ContentBounds.max / 2;
            ...
            if (m_Horizontal)
            {
                ...
                if (min.x > m_ViewBounds.min.x)
                    offset = m_ViewBounds.min.x - min.x;
                else if (max.x < m_ViewBounds.max.x)
                    offset = m_ViewBounds.max.x - max.x;
            }
